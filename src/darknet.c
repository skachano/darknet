...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    if (calc_map) {
        ...
        printf(" Prepare additional network for AP calculation...\n");
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            iter_map = iteration;
            mean_average_precision = validate_detector_obstacle_ap(datacfg, cfgfile, weightfile, 0.25, 0.5, 0, net.letter_box, &net_map);// &net_combined);
...
float validate_detector_obstacle_ap(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
